...
//#include "libspeex/speex.h" 
	#include "libspeex2/include/speex/speex.h" 

}

#define NARROW_SAMPLES_PER_FRAME       160/*160*/
#define WIDE_SAMPLES_PER_FRAME         320

#define NS_PER_FRAME                   10000000
#define C_16000         16000
...
static void * create_encoder(const struct PluginCodec_Definition * codec)
{
  ...
  speex_encoder_ctl(context->coderState, SPEEX_SET_QUALITY,    &mode);
	int bitrate = 1600;
  speex_encoder_ctl(context->coderState, SPEEX_SET_BITRATE,    &bitrate);
	int complexity = 6;
  speex_encoder_ctl(context->coderState, SPEEX_SET_COMPLEXITY,    &complexity);
 return context;
}
...
#define DECLARE_SIP_SPEEX_CODEC(name,des, clockRate, suffix, ordinal, bitsPerFrame, samplesPerFrame) \
{ \
  /* encoder */ \
  PLUGIN_CODEC_VERSION,               /* codec API version */ \
  &licenseInfo,                       /* license information */ \
  ...
  PluginCodec_InputTypeRaw |          /* raw input data */ \
  PluginCodec_OutputTypeRaw |         /* raw output data */ \
  PluginCodec_RTPTypeExplicit |         /* share RTP code */ \
          /* dynamic RTP type */ \
  PluginCodec_DecodeSilence,          /* can encode silence frames */ \
  des,                               /* text decription */ \
  L16Desc,                            /* source format */ \
  ...
  NS_PER_FRAME,                       /* nanoseconds per frame */ \
    ...
    ((bitsPerFrame + 7) / 8),             /* bytes per frame */ \
    1/*1*/,                                  /* recommended number of frames per packet */ \
    1/*1*/,                                  /* maximum number of frames per packet  */ \
  C_SPEEX_PT,                                  /* IANA RTP payload code */ \
...
////#endif
 ...
 // DECLARE_SIP_SPEEX_CODEC("G.729"/*"SpeexNB"*/,"speex" , 8000, Narrow8k, 7/*3*/, 160/*NARROW_BITSPERFRAME_MODE3*/, NARROW_SAMPLES_PER_FRAME),
  DECLARE_SIP_SPEEX_CODEC( "SpeexWB","speex" , 16000, Wide20k6, 7, 412/*WIDE_BITSPERFRAME_MODE6*/,   WIDE_SAMPLES_PER_FRAME)
};

#define NUM_VER2_DEFNS   (sizeof(ver2SpeexCodecDefn) / sizeof(struct PluginCodec_Definition))

/////////////////////////////////////////////////////////////////////////////
 void* g_GetcodecdefPointerSpeexCodec(int * count)
{
	int i= WIDE_BITSPERFRAME_MODE6;
  *count = sizeof(ver2SpeexCodecDefn)/ sizeof(struct PluginCodec_Definition);
  return &ver2SpeexCodecDefn[0];
