...
__inline static double GetReal(const PString & fieldValue, const PCaselessString & key, double dflt = 0)
{
  PString str = SIPMIMEInfo::ExtractFieldParameter(fieldValue, key);
  return !str.IsEmpty() ? str.AsReal() : dflt;
}

__inline static int GetInteger(const PString & fieldValue, const PCaselessString & key, int nflt = 0)
{
  PString str = SIPMIMEInfo::ExtractFieldParameter(fieldValue, key);
  return !str.IsEmpty() ? str.AsInteger() : nflt;
}
