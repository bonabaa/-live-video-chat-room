 ...
 * $Revision: 23944 $
 * $Author: rjongbloed $
 * $Date: 2010-01-18 10:13:28 +0000 (Mon, 18 Jan 2010) $
 */
...
class SIPConnection : public OpalRTPConnection
{
  ...
  public:
    PString m_strOnReceivedSDPMediaDescription;
    unsigned int m_AudiotimestampForSend;//it is set by audio RTP session ,for video session using
    unsigned int m_AudiotimestampForRecv;;
  /**@name Construction */
  //@{
    ...
    ~SIPConnection();
  //@}
		void SetForceAddVideoSDP(bool b)  { m_bForceAddVideoSDP=b; if (b) SetRemoteMediaFormats(NULL);}
  ...
  //@{
    ...
    virtual void OnReceivedUPDATE(SIP_PDU & pdu);
    virtual PBoolean OnReceivedSessionIntervalTooSmall(SIPTransaction & transaction, SIP_PDU & pdu);
		virtual void startRefresher(){PAssertNULL(0);};
		virtual void stopRefresher(){PAssertNULL(0);};
  ...
  //@}
	...
	void	SetOnlyAudioCall(bool bOnly) { m_bOnlyAudioCall = bOnly; }
#endif
  ...
  protected:
    ...
    void CheckIfStartSessionTimeer(bool bCallerOrCalee, SIPMIMEInfo& mime , SIP_PDU& response);
	...
	//bool					  m_handlingINVITE;
	bool						m_bNeverSendReinvite;
	bool						m_bOnlyAudioCall;
	bool						m_bForceAddVideoSDP;
	bool						m_bSupport100Rel;
	bool						m_bMSRPConnection;
    ...
    OpalMediaFormatList m_answerFormatList;
		bool m_bEnableSessionTimer;// session expires >= m_nMinSE
		bool m_bTimerRefresher;
		bool m_bEnablePrivacy;// session expires >= m_nMinSE
		unsigned int m_nSessionExpires;// session expires >= m_nMinSE
		unsigned int m_nMinSE;//min session expires ,>=90
		PCaselessString m_strRefresher;// uac or uas 
...
#define D_ONRECEIVEDSDPMEDIADESCRIPTION "OnReceivedSDPMediaDescription"
#endif // OPAL_SIP
