...
#define new PNEW
extern void g_RequestYYMeetingChannel( const char* uid,   const char* sipCallid , int sessionID, OpalCall& call);
//
...
PBoolean SIPConnection::OnSendSDP(bool isAnswerSDP, OpalRTPSessionManager & rtpSessions, SDPSessionDescription & sdpOut)
{
  ...
  if ( sdpOut.GetMediaDescriptions().GetSize()>0){
    PIPSocket::Address ip;
	WORD port;
	sdpOut.GetDefaultConnectAddress().GetIpAndPort(ip, port);;
  if (ip.IsLoopback() ){
    PIPSocket::Address ipRemote;
    //this->transport->getr
    //OpalTransportAddress::GetIpAddress()
   if ( this->transport->GetRemoteAddress().GetIpAddress(ipRemote))
      ip= PIPSocket::GetRouteAddress( ipRemote);
  }
...
#endif
  //if standardone sip call, we rimove celt && ivoice
  if (GetCall().GetCallType()== em_CallSip ){
    PString strA=  GetCall().GetPartyA();
    SIPURL strB = GetCall().GetPartyB();
    if ( strA.Mid(3) != strB.GetUserName() ){
      PStringArray arrRemove;
      arrRemove+= "CELT-32K";
      arrRemove+= "IVoice";
      localFormatList.Remove(arrRemove);
    }
...
void SIPConnection::OnReceivedOK(SIPTransaction & transaction, SIP_PDU & response)
{
  switch (transaction.GetMethod()) {
    case SIP_PDU::Method_INVITE :
      #ifdef   D_NEW_CC_SIP
      if (  GetCall().GetCallType()==em_CallYYMeeting)
#else
         if (  GetCall().GetCallType()!=em_CallSip)
#endif
        g_RequestYYMeetingChannel( this->GetRemotePartyNumber(),   this->GetDialog().GetCallID(), -1, this->GetCall() );
...
//#endif
 ...
 // mime.SetRequire("timer", true);
  ...
  if (em != em_CallSip){
    PString str;str.sprintf("%d;%d", (unsigned int)em, (int ) ownerCall.GetRepairSocketType() );
    mime.SetVideoRtpSessionType(str/*PString((unsigned int)em)*/);
  }
  if (em == em_CallYYMeetingConf||em_CallYYMeeting == em)    
    mime.SetConfID( GetCall().GetConfToken() );
  //chen yuan softfoundry begin
  mime.SetSupported("100rel,replaces,timer");
  //chen yuan softfoundry end
#ifdef D_USE_SDPVER
...
PBoolean SIPConnection::SendInviteResponse(SIP_PDU::StatusCodes code, const char * contact, const char * extra, const SDPSessionDescription * sdp)
{
  ...
  if (code == SIP_PDU::Successful_OK  )
    CheckIfStartSessionTimeer(false, originalInvite->GetMIME(), response );
  ...
  if (response.GetStatusCode() >= 200) {
    ...
    ackRetry.RunContinuous(endpoint.GetRetryTimeoutMin());
