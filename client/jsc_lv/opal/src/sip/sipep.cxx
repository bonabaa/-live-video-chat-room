...
OpalTransport * SIPEndPoint::CreateMessageTransport(const SIPURL & remoteURL, const SIPURL & remoteParty)
{
  OpalTransportAddress remoteAddress = remoteURL.GetHostAddress();

  OpalTransportAddress localAddress;
  PSafePtr<SIPHandler> handler = activeSIPHandlers.FindSIPHandlerByDomain(remoteParty.GetHostName(), SIP_PDU::Method_REGISTER, PSafeReadOnly);
  if (handler != NULL) {
    OpalTransport * transport = handler->GetTransport();
	if (transport != NULL) {
	  localAddress = transport->GetInterface();
	  PTRACE(4, "SIP\tFound registrar on domain " << remoteParty.GetHostName() << ", using interface " << transport->GetInterface());
	}
  }
...
PBoolean SIPEndPoint::OnReceivedINVITE(OpalTransport & transport, SIP_PDU * request)
{
  ...
  if (transport.IsReliable())
   ...
   SIPURL  sipurl=  SIPURL(mime.GetFrom(), transport.GetRemoteAddress(), 0);
   
   sipurl.SetPort(remotePort);
		newTransport = CreateTransport(sipurl, transport.GetInterface());
  ...
  if (call == NULL) {
    ...
    //chenyuan
    em_CallType ntype= em_CallSip;
    em_rtp_type rtptype= em_standard_udp;
    if (mime.Contains("RtpSessionType")){
      ntype =   (em_CallType) mime.GetVideoRtpSessionType()  ;
       
      PStringArray arr= mime.GetString("RtpSessionType").Tokenise(";");
      if (arr.GetSize()>=2 ){
        //get the udt style
        rtptype=(em_rtp_type)arr[1].AsInteger();
    ...
    call->SetRepairSocketType(rtptype);
    //}
    if (call == NULL) {
  ...
  if (call->GetCallType() == em_CallYYMeetingConf || call->GetCallType() == em_CallYYMeeting)
...
PBoolean SIPEndPoint::Message(const PURL & to, const PString & type, const PString & body, PURL & from, PString & conversationId)
{
  ...
  if (handler != NULL) {
    ...
    SIPMessage::Params params;//+	PAbstractArray	{elementSize=1 theArray=0x077a0f18 "+862062327640
//ims.gd.chinamobile.com" allocatedDynamically=true }	const PAbstractArray

	PSafePtr<SIPHandler> handlerReg = activeSIPHandlers.FindSIPHandlerByDomain(from.IsEmpty() ? to.GetHostName() : from.GetHostName(), SIP_PDU::Method_REGISTER, PSafeReadWrite);
    if (handlerReg ){
        SIPRegister::Params  paramsReg =  ((SIPRegisterHandler *)&*handlerReg)->GetParams();
        params.m_contactAddress= paramsReg.m_contactAddress;
        params.m_localAddress    = paramsReg.m_localAddress;
	    ...
	    params.m_proxyAddress    = GetProxy();
	    if( params.m_proxyAddress.IsEmpty() ) {
		    params.m_proxyAddress    = paramsReg.m_proxyAddress;
	    }
