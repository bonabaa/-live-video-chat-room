 ...
 * $Revision: 26386 $
 * $Author: rjongbloed $
 * $Date: 2011-09-01 10:48:35 +0800 (周四, 01 九月 2011) $
 */
...
//const GUID  MEDIASUBTYPE_HDYC = {0x43594448, 0x0000, 0x0010, {0x80, 0x00, 0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71}} ;
  ...
  class PVideoInputControl_DirectShowEx : public PVideoInputControl
  {
      PCLASSINFO(PVideoInputControl_DirectShowEx, PVideoInputControl);
    public:
      PVideoInputControl_DirectShowEx(IAMCameraControl * _pCC);
...
#ifndef _WIN32_WCE
  dwMsgLen = AMGetErrorTextA(hr, msg.GetPointer(), msg.GetSize());
  if (dwMsgLen > 0) {
    msg.MakeMinimumSize();
    return msg;
  }
...
PStringArray PVideoInputDevice_DirectShowEx::GetInputDeviceNames()
{
  PVideoInputDevice_DirectShowEx instance;
  return instance.GetDeviceNames();
}


PBoolean PVideoInputDevice_DirectShowEx::GetDeviceCapabilities(const PString & deviceName,
...
bool PVideoInputDevice_DirectShowEx::GetDeviceCapabilities(Capabilities * caps) const
{
  ...
  for (std::vector<PVideoFrameInfo*>::iterator it = fsizes.begin(); it != fsizes.end(); ++it) {
    PVideoFrameInfo* p=*it;

    PTRACE(4,"PVidDirectShow2010\tFmt["<< 0 << "] = ("
                << p->GetColourFormat() << ", "
                << p->GetFrameWidth() << "x" << p->GetFrameHeight() << ", "
                << p->GetFrameRate() << "fps)");

   // PTRACE(5, "DShow\tFormat["<< caps->framesizes.size() << "] = (" << **it  <<"width=["<< p->GetFrameWidth() <<"],Height=["<< p->GetFrameHeight()<<"]" );
...
bool PVideoInputDevice_DirectShowEx::SetPinFormat(unsigned useDefaultColourOrSize)
{
...
PBoolean PVideoInputDevice_DirectShowEx::Open(const PString & devName,
...
PBoolean PVideoInputDevice_DirectShowEx::SetColourFormat(const PString & newColourFormat)
{
...
PBoolean PVideoInputDevice_DirectShowEx::SetFrameRate(unsigned newRate)
{
...
bool PVideoInputDevice_DirectShowEx::FlowControl(const void * flowData)
{
...
PBoolean PVideoInputDevice_DirectShowEx::GetFrameDataNoDelay(BYTE * destFrame, PINDEX * bytesReturned)
{
...
int PVideoInputDevice_DirectShowEx::GetControlCommon(long control)
{
...
int PVideoInputDevice_DirectShowEx::GetBrightness()
{
  return GetControlCommon(VideoProcAmp_Brightness);
}

int PVideoInputDevice_DirectShowEx::GetWhiteness()
{
  return GetControlCommon(VideoProcAmp_Gamma);
}

int PVideoInputDevice_DirectShowEx::GetColour()
{
  return GetControlCommon(VideoProcAmp_Saturation);
}

int PVideoInputDevice_DirectShowEx::GetContrast()
{
  return GetControlCommon(VideoProcAmp_Contrast);
}

int PVideoInputDevice_DirectShowEx::GetHue()
{
  return GetControlCommon(VideoProcAmp_Hue);
}

PBoolean PVideoInputDevice_DirectShowEx::GetParameters(int *whiteness, int *brightness, int *colour, int *contrast, int *hue)
{
  if (!IsOpen())
    return false;
...
PBoolean PVideoInputDevice_DirectShowEx::SetControlCommon(long control, int newValue)
{
...
PBoolean PVideoInputDevice_DirectShowEx::SetBrightness(unsigned newBrightness)
{
  return SetControlCommon(VideoProcAmp_Brightness, newBrightness);
}

PBoolean PVideoInputDevice_DirectShowEx::SetColour(unsigned newColour)
{
  return SetControlCommon(VideoProcAmp_Saturation, newColour);
}

PBoolean PVideoInputDevice_DirectShowEx::SetContrast(unsigned newContrast)
{
  return SetControlCommon(VideoProcAmp_Contrast, newContrast);
}

PBoolean PVideoInputDevice_DirectShowEx::SetHue(unsigned newHue)
{
  return SetControlCommon(VideoProcAmp_Hue, newHue);
}

PBoolean PVideoInputDevice_DirectShowEx::SetWhiteness(unsigned newWhiteness)
{
...
PINDEX PVideoInputDevice_DirectShowEx::GetCurrentBufferSize()
{
  return m_pSampleGrabber->m_sampleSize;
}


bool PVideoInputDevice_DirectShowEx::GetCurrentBufferData(BYTE * pData)
{
...
bool PVideoInputDevice_DirectShowEx::BindCaptureDevice(const PString & devName)
{
...
bool PVideoInputDevice_DirectShowEx::PlatformOpen()
{
  HRESULT hr = S_OK;
...
PVideoInputControl_DirectShowEx::PVideoInputControl_DirectShowEx(IAMCameraControl * _pCC)
  : t_pCC(_pCC)
{
  HRESULT  hr;
  PVideoControlInfo panInfo;
...
bool PVideoInputControl_DirectShowEx::Pan(long value, bool absolute)
{
...
bool PVideoInputControl_DirectShowEx::Tilt(long value, bool absolute)
{
  PWaitAndSignal m(ccmutex);
...
bool PVideoInputControl_DirectShowEx::Zoom(long value, bool absolute)
{
  PWaitAndSignal m(ccmutex);
...
bool PVideoInputDevice_DirectShowEx::GetCurrentBufferData(BYTE * data)
{
...
PVideoInputControl * PVideoInputDevice_DirectShowEx::GetVideoInputControls()
{
  if (m_pCameraControls != NULL)
    return new PVideoInputControl_DirectShowEx(m_pCameraControls);
  return NULL; 
}
