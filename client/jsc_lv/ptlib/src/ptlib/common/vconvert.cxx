 ...
 * $Revision: 22480 $
 * $Author: rjongbloed $
 * $Date: 2009-04-29 06:38:10 +0000 (Wed, 29 Apr 2009) $
 */
...
//  Ipp8u* pDst[3]={yplane,uplane,vplane};
  //int dstStep[3]={srcFrameWidth,srcFrameWidth>>1, srcFrameWidth>>1 };
//  IppiSize roiSize={srcFrameWidth,srcFrameHeight };   // int j=0;  int srcStep=srcFrameWidth*rgbIncrement;//3 or 4?  // int j=0  //if ( 1 )
  //{
  //  ;//ippiBGRToYCbCr420_8u_C3P3R(rgb + (srcFrameHeight-1) * srcStep, -srcStep, pDst, dstStep, roiSize);  //}
  //else
   /* if (redOffset ==2 && blueOffset==0&&rgbIncrement==3 )
    {
      ippiBGRToYCbCr420_8u_C3P3R(rgb + (srcFrameHeight-1) * srcStep, -srcStep, pDst, dstStep, roiSize);
    }
    else*/
...
void  PStandardColourConverter::YUY2toYUV420PSameSize(const BYTE *yuy2, BYTE *yuv420p) const
{
  ...
  v = u + npixels/4;
  //src
  const Ipp8u* pSrc = yuy2;
  int  srcStep=srcFrameWidth*2 ;
  //dest
  Ipp8u* pDst[3]={y,u,v};
  int dstStep[3]={srcFrameWidth,srcFrameWidth>>1, srcFrameWidth>>1 };
  IppiSize roiSize={srcFrameWidth,srcFrameHeight };
...
void  PStandardColourConverter::UYVY422toYUV420PSameSize(const BYTE *uyvy, BYTE *yuv420p) const
{
  ...
  u = yuv420p + npixels;
  v = u + npixels/4;
//ippiCbYCr422ToYCbCr420_8u_C2P3R
 //src
  const Ipp8u* pSrc = uyvy;
  int  srcStep=srcFrameWidth*2 ;
  //dest
  Ipp8u* pDst[3]={y,u,v};
  int dstStep[3]={srcFrameWidth,srcFrameWidth>>1, srcFrameWidth>>1 };
...
//ImageYUY2 = ippiMalloc_8u_C2( roiSize.width, roiSize.height, &stepYUY2 );
  ...
  ippiCbYCr422ToYCbCr420_8u_C2P3R(  pSrc,     srcStep ,  pDst ,   dstStep ,  roiSize);
#else
  const BYTE *s;
  BYTE *y, *u, *v;
  unsigned int x, h;  
...
#ifdef D_TINY_JPG// defined (__GNUC__) || defined (__sun)
#ifndef P_MACOSX
/*
                                              ...
                                              PINDEX *bytesReturned)
{
  ...
  if ((srcFrameWidth == dstFrameWidth) && (srcFrameHeight == dstFrameHeight)) {
    ...
    if (MJPEGtoYUV420PSameSize(mjpeg, yuv420p) == false){
       PTRACE(2,"PColCnv\tMJPEG to YUV420P failed\n");
       return false;
    }
