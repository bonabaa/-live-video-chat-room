...
PObject::Comparison PAbstractSortedList::Compare(const PObject & obj) const
{
  ...
  while (elmt1->left != &info->m_nil)
    elmt1 = elmt1->left;

  Element * elmt2 = ((const PAbstractSortedList &)obj).info->root;
  while (elmt2->left != &info->m_nil)
    elmt2 = elmt2->left;

  while (elmt1 != &info->m_nil && elmt2 != &info->m_nil) {
    if (elmt1 == &info->m_nil)
      return LessThan;
    if (elmt2 == &info->m_nil)
      return GreaterThan;
...
PINDEX PAbstractSortedList::Append(PObject * obj)
{
  ...
  z->parent = z->left = z->right = &info->m_nil;
  z->colour = Element::Black;
  z->subTreeSize = 1;
  z->data = obj;

  Element * x = info->root;
  Element * y = &info->m_nil;
  while (x != &info->m_nil) {
    x->subTreeSize++;
    y = x;
...
PINDEX PAbstractSortedList::GetObjectsIndex(const PObject * obj, PSortedListElement * & lastElement) const
{
  ...
  if (elmt->data != obj) {
    ...
    while (elmt->data != obj &&
            (elmt = info->Predecessor(elmt)) != &info->m_nil &&
            *obj == *elmt->data)
      pos--;
    if (elmt->data != obj) {
      pos = savePos;
      elmt = saveElmt;
      while (elmt->data != obj &&
              (elmt = info->Successor(elmt)) != &info->m_nil &&
              *obj == *elmt->data)
        pos++;
      if (elmt->data != obj)
        return P_MAX_INDEX;
...
PINDEX PAbstractSortedList::GetValuesIndex(const PObject & obj) const
{
  ...
  Element * prev;
  while ((prev = info->Predecessor(lastElement)) != &info->m_nil &&
...
void PAbstractSortedList::RemoveElement(Element * node)
{
  ...
  if (PAssertNULL(node) == &info->m_nil)
    return;

  if (node->data != NULL && reference->deleteObjects)
    delete node->data;

  Element * y = node->left == &info->m_nil || node->right == &info->m_nil ? node : info->Successor(node);
  ...
  Element * x = y->left != &info->m_nil ? y->left : y->right;
  x->parent = y->parent;

  if (y->parent == &info->m_nil)
    info->root = x;
...
void PAbstractSortedList::LeftRotate(Element * node)
{
  ...
  if (pivot->left != &info->m_nil)
    pivot->left->parent = node;
  pivot->parent = node->parent;
  if (node->parent == &info->m_nil)
    info->root = pivot;
...
PSortedListElement * PSortedListInfo::Successor(const PSortedListElement * node) const
{
  ...
  if (node->right != &m_nil) {
    next = node->right;
    while (next->left != &m_nil)
      next = next->left;
  }
  else {
    next = node->parent;
    while (next != &m_nil && node == next->right) {
      node = next;
      next = node->parent;
    }
  }
...
{
  if (node->left != &info->m_nil) {
    DeleteSubTrees(node->left, deleteObject);
    delete node->left;
    node->left = &info->m_nil;
  }
  if (node->right != &info->m_nil) {
    DeleteSubTrees(node->right, deleteObject);
    delete node->right;
    node->right = &info->m_nil;
  }
  if (deleteObject) {
    delete node->data;
    node->data = NULL;
