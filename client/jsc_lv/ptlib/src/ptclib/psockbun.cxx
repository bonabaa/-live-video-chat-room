 ...
 * $Revision: 23936 $
 * $Author: rjongbloed $
 * $Date: 2010-01-14 02:36:07 +0000 (Thu, 14 Jan 2010) $
 */
...
//}
//
//bool g_compress(const char* src,const uLongf slen, Byte* dst,  uLongf*  dlen)
//{
//    int err;
//     
//    *dlen = 1300;
//    err = compress(dst, dlen, (const Bytef*)src, slen);
//    if (err!= Z_OK)
//      return false;
//    else
//      return true;
...
//};
//bool g_uncompress(const char* src,const uLongf slen, Byte* dst,  uLongf*  dlen)
//{
//    int err;
//    *dlen = 1300;
//    err = uncompress(dst, dlen,(const Bytef*) src, slen);
//    if (err!= Z_OK)
//      return false;
//    else
//      return true;
...
//extern bool g_YYSocketWriteToWithHeader(PUDPSocket& socket, const PIPSocket::Address& addr, WORD port ,const char* framePtr,const int frameSize, int& nWriteCount, int originalbuflen);
...
#if 1
	//if (info.socket->Listen(10,  localPort, PIPSocket::AddressIsExclusive )){
	if (info.socket->Listen(binding, 0, localPort, false/*reuseAddress chen yuan*/?PIPSocket::CanReuseAddress:PIPSocket::AddressIsExclusive)) {
#else
  if (info.socket->Listen( 0, localPort, false?PIPSocket::CanReuseAddress:PIPSocket::AddressIsExclusive)) {
#endif
...
//	{
//		//chen yuan
//		char bufencode[1500]={0};
//		unsigned int bufencodelen=sizeof(bufencode);
//		if (BZ2_bzBuffToBuffCompress(bufencode , &bufencodelen ,(char*)buf, len , 9,0,0)==0/*g_compress((const char*)buf,len, bufencode, &bufencodelen)*/)
//		{
//	#if 0
//			info.socket->WriteTo(bufencode, bufencodelen, addr, port);
//			lastWriteCount = (info.socket->GetLastWriteCount()== bufencodelen)? len:0;
//	#else
//			//support to write msg VIA BBQChanel
//			int nWriteCount=0;
//			//CMyClient* pMe=NULL;
//			if ( m_pMgr)
//			{
//#ifndef D_XMEETING
//				((CMyClient*)m_pMgr)->YYSocketWriteToWithHeader(*info.socket, addr, port,(const char*)bufencode, bufencodelen, nWriteCount, len);
//#endif
//			}else
//			{
//				PAssertNULL(0);
//			}
//			lastWriteCount = (info.socket->GetLastWriteCount()== nWriteCount)? len:0;
//	#endif
//		}else
//		{
//			info.socket->WriteTo(buf, len, addr, port);
                                                   ...
                                                   const PTimeInterval & timeout)
{
  ...
  socket = (PUDPSocket *)&readers.front();
	...
	//if (m_pMgr== NULL) 
	{
		bReadOK = socket->ReadFrom(buf, len, addr, port);
		if (bReadOK)
		{
	...
	//	#else
	//		CMyClient* pMe= (CMyClient*)m_pMgr;
	//		bReadOK=pMe->YYSocketReadFromWithHeader( *socket, (char*)buf, len,  lastReadCount, addr, port );
	//	#endif
	//	if (bReadOK)
	//	{
	//		//chenyuan
	...
	//		if (/*socket->GetLastReadCount()*/ lastReadCount >0 && 0==BZ2_bzBuffToBuffDecompress(bufencode, &bufencodelen ,(char*)buf, lastReadCount , 0, 0)/* g_uncompress((const char*)buf, lastReadCount, bufencode, &bufencodelen)*/)
	//		{
	//			memcpy( buf, bufencode,bufencodelen);
	//			lastReadCount = bufencodelen;
	//		}else
	//			lastReadCount = lastReadCount/*socket->GetLastReadCount()*/;
...
//	{
//#ifndef D_XMEETING
//		int nType = ((CMyClient*)pMgr)->m_nListenType;
//#else
//		int nType = 1;
//#endif
//		if (nType ==1 /*sip*/ || nType == 2 /*h323*/)
//		{
//			pMgr =NULL;
//		}else if (nType ==0 /*yy*/ )
//		{
