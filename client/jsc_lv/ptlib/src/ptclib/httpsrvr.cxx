...
{
  PFile & file = ((PHTTPFileRequest&)request).file;
  //if(file.GetFilePath().Right(4) ==".xap"&& m_bufferFile.GetSize() >0  ){
  //  return false;
  //}
...
#else
  if ( file.GetFilePath().Right(4) !=".xap"){
    //chy
    if (count > 10000)
      count = 10000;
    if (count > 0){
    ...
    if( m_bufferFile.GetSize() <=0 ){
      if (count > 0){
        PAssert(file.Read(data.GetPointer(count), count), PLogicError);
        memcpy(m_bufferFile.GetPointer(count), data.GetPointer(), data.GetSize());
        file.Close();
        return PFalse;
      }
    }
    else{
      memcpy(data.GetPointer(m_bufferFile.GetSize()), m_bufferFile.GetPointer(), m_bufferFile.GetSize());
      file.Close();
      return PFalse;
    }
  }
