 ...
 *
 *
 * CCFile Input Implementation for the PTLib Library.
 *
...
struct st_CCFileHeader
{
  int w,h,nfps, nDatabock;
};

//////////////////////////////////////////////////////////////////////
// Video Input device

/**This class defines a video input device for CCFile sharing on Windows.
 */

class PVideoInputDevice_CCFile : public PVideoInputDevice
{
  PCLASSINFO(PVideoInputDevice_CCFile, PVideoInputDevice);

  public:
    static PFile* g_recordfile;

    static bool CCRecoreFile_Open(int h, int w, int nfps, int nDataBlock);
    static bool CCRecoreFile_Write(void * data, int datalen);
    static bool CCRecoreFile_Close();
    /** Create a new video input device.
     */
    PVideoInputDevice_CCFile();
  ...
  protected:
      PAdaptiveDelay grabDelay;  ///< Frame Grab delay
			st_CCFileHeader m_header;
      PFile m_file;
 
};


//PWLIB_STATIC_LOAD_PLUGIN(CCFile, PVideoInputDevice);
