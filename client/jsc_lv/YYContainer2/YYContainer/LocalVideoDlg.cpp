...
BEGIN_MESSAGE_MAP(CLocalVideoDlg, CDialog)
  ...
  ON_WM_CONTEXTMENU()
  ON_WM_INITMENUPOPUP()
	ON_COMMAND_RANGE(ID_VIDEORATE_64K, ID__MAXIMIZE,OnMenuItem)
...
void CLocalVideoDlg::OnMenuItem(UINT nID) 
{
 // int nRemoteID = atoi(m_strRemoteUserID.GetBuffer());
    ...
    for(std::list<stMsgVideoFrame>::iterator i = m_listVideoSize.begin(); i !=m_listVideoSize.end(); i++,j++)
        ...
        CString strParam = "";
        strParam.Format("20;2;%d;%d;%d",  w,h, bitrate );//em_msgfromcontainer
    ...
    case ID_VIDEORATE_2048K:
      ...
      CString strParam = "";
      strParam.Format("20;3;%d;",   bitrate );//em_msgfromcontainer
      g_CallJsScript(m_pContainer, strParam);
    ...
    CString strParam = "";
    strParam.Format("20;4;%d", framerate );//em_msgfromcontainer
    g_CallJsScript(m_pContainer, strParam);
...
void CLocalVideoDlg::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
  ...
  CString strVideoSizes= CString(ret.bstrVal);
  int nstart=0;
  CString strValue;
  char tmp[24]={0};
  while ( (strValue=strVideoSizes.Tokenize(",", nstart) )!="" ){
    ...
    if (i->frameWidth% 16 ==0 &&   i->frameHeight%16==0)
      ...
      message.Format("%dx%d", i->frameWidth, i->frameHeight );
			m_popupMenu->AppendMenuA(MF_STRING, IDC_MENEU_VIDESIZE   + j++, message.GetBuffer() );

		}
	}
  pPopup->AppendMenuA(MF_POPUP, (UINT_PTR)m_popupMenu->operator HMENU(), "视频大小");
